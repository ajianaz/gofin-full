...
export const categoryService = {
	async list(): Promise<Category[]> {
		...
		return unwrapMany<Category>(res).map((c) => ({
			...c,
			type: (c as any).type ?? 'expense',
			transaction_count: (c as any).transaction_count ?? 0
		}));
	},
	...
	async create(data: { name: string }): Promise<Category> {
		...
		return { ...c, type: (c as any).type ?? 'expense', transaction_count: (c as any).transaction_count ?? 0 };
	},
