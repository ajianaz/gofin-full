...
func (h *WebhookHandler) Index(c *fiber.Ctx) error {
	...
	webhooks, total, err := h.repo.ListPaginated(c.Context(), *groupID, page, perPage)
	if err != nil {
		log.Error().Err(err).Msg("handler: failed to list webhooks")
...
func (h *WebhookHandler) Store(c *fiber.Ctx) error {
	...
	if err != nil {
		log.Error().Err(err).Msg("handler: failed to create webhook")
		return apperrors.ErrInternal
	}
...
func (h *WebhookHandler) Messages(c *fiber.Ctx) error {
	...
	if err != nil {
		log.Error().Err(err).Msg("handler: failed to list webhook messages")
		return apperrors.ErrInternal
	}
